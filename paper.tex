...
\subsection{Construction of Prompt Template}
	...
	The template adopts a generalized structure, where the general requirement is first stated in a concise and natural way (e.g., "Please list all the entities mentioned below."). Secondly, the format and content requirements are detailed and subdivided by means of separate specific requirement sentences, using appropriate verbs and giving formatting paradigms where necessary to enable the larger model to better grasp the outcome requirements (e.g., "For each entity, provide a sentence structure in the form of "X is/means..."). " sentence structure to provide a short explanation or definition, where X refers to the entity or concept appearing in the text.") Finally, the model is tested several times to see if there are any deviations in the understanding of the requirements and to modify or add clarifications to the requirements that are ambiguous to the model (e.g., adding at the end of the prompt, "Please return the answer in the following form, being careful not to repeat it: \textbackslash n\textbackslash nA is ..... \textbackslash nB is ....") . In addition, when the prompt text is long, in order to avoid forgetting or confusing the requirements in the larger language model, specific requirements that do not perform well on the test can be re-emphasized in a separate paragraph distinct from the overall requirement.
