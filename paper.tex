...
\begin{itemize}
	...
	\item We propose a new calculation method for the overlap F1 score in multi-span QA task to alleviate the issue of lower scores in situations where the model decomposes the answer into multiple complete spans while the answer spans are still overall correct.The adjusted metrics provides a more accurate reflection of the model’s performance in handling complex multi-span answers.
...
\subsection{Information Injection}
	For the entity interpretation data, the enhanced knowledge is injected into the original data (questions and paragraphs) by means of insertion interpretation. Specifically, the entity interpretation data is first parsed by an automated script, which on the one hand parses most of the sentence structures by regular expressions; on the other hand, for a small number of sentences that deviate from the template specification, the entities and their interpretations are identified by using the Semantic Dependency Analysis model in Spacy. After obtaining the ``Entity-Entity Interpretation" knowledge base for each QA data, the original data is scanned for questions and paragraphs, and the corresponding explanation is inserted as ``Entity (Entity and its Interpretation)" after each entity (if it is in the Entity and its Interpretation base of the data), and the corresponding explanation is enclosed in parentheses. The corresponding explanation is inserted after each entity (if it is in the database of entities and their explanations) in the form of ``Entity (Entity and its explanation)", and the inserted explanation of the entity is surrounded by parentheses to make sure that the augmented text injected with the knowledge is still a natural sentence or paragraph that flows smoothly (e.g., ``how long does it take for the milky way (Milky Way: The galaxy that contains our Solar System and is home to billions of stars.) to rotate?").
	...
	For entity-relationship analysis and content continuation knowledge, the augmented knowledge is injected into the original paragraph using random slice splicing in proportion to its length. Firstly, the ratio of the length of the enhanced knowledge text to the original paragraph is calculated; secondly, in the input layer of the model, the original input data is sliced into text fragments with a length of 512*original length/(original length+enhanced text length), and the enhanced text is sliced into text fragments with a length of 512*enhanced text length/(original length+enhanced text length); then, randomly select the enhanced text fragments, and splice them Then randomly select the enhanced text fragment, splice it into the middle of each original text fragment, and finally input this long text into the model by sliding, in order to ensure that the original data can fully interact with the enhanced data in the bottom position of the model, as shown in Figure\ref{fig:random_concatenate}.
